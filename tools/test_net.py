...
class Trainer(DefaultTrainer):
    ...
    def build_evaluator(cls, cfg, dataset_name, output_folder=None):
        ...
        if evaluator_type == "api_dataset":
            return ApiEvaluator(dataset_name)
...
def main(args):
    ...
    if args.eval_only:
        ...
        if comm.is_main_process():
            ...
            upload_conf_score = []
            upload_class = []
            for cls_ in res.keys():
                for prediction in res[cls_]:
                    upload_class.append(cls_)
                    tmp = prediction.split()
                    upload_file_id.append(tmp[0])
                    upload_conf_score.append(float(tmp[1]))
                    upload_bbox.append([float(tmp[2]), float(tmp[3]), float(tmp[4]), float(tmp[5])])
            ...
            with open(os.path.join(cfg.OUTPUT_DIR, 'inference', 'res_final.csv'), 'w') as fp:
                df.to_csv(fp)
        return res
    elif args.eval_all:
        tester = Tester(cfg)
