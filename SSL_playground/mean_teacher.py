...
class MeanTeacher(UDA):
    ...
    def training_step(self, batch_list, batch_inx):
        ...
        unlab_pred_student = self.student.forward(unlab_x)

        sup_loss = nll_loss(log_softmax(unlab_pred_student, dim=1), unlab_y)

        with torch.no_grad():
            unlab_pred_teacher = self.net.forward(unlab_x)
        ...
        unsup_loss = mse_loss(log_softmax(unlab_pred_student, dim=1), log_softmax(unlab_pred_teacher, dim=1))
