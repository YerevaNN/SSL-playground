...
from SSL_playground.cifar.datasets import get_train_test_datasets as get_cifar_train_test_datasets
...
def get_uda2_train_test_loaders(dataset_name, path, num_labelled_samples, batch_size, num_workers,
    ...
    if "cifar" in dataset_name.lower():
        ...
        original_transform = lambda dp: train_transform(dp[0])
        augmentation_transform = lambda dp: unsupervised_train_transformation(dp[0])
        train1_unsup_ds = TransformedDataset(train1_ds, UDATransform(original_transform, augmentation_transform))
        train2_unsup_ds = TransformedDataset(train2_ds, UDATransform(original_transform, augmentation_transform))
