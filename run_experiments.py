    ...
    for phase in phases:
        ...
        for stage, budget in enumerate(task['budgets']):
            ...
            if phase == 'adaption':
                data_id += 8
            data_id += 7 # TODO
            cos_id = int(data_id / 4)
            sc_id = int(data_id % 4)

            print('YEEEET, {} {}'.format(cos_id, sc_id))
            data_path = os.path.join('./session_data', 'experiment_ckpt_28', 'cos{}_sc{}'.format(cos_id, sc_id))

            cmd = 'python run_one_experiment.py --output_csv {} --session_id {} --dataset_name {} --phase {} --stage {} --class_num {} --ckpt_path {} --data_path {}'.format(output_csv, task['session_token'], task['session']['Session_Status']['current_dataset']['name'], phase, 4, len(class_id_to_name.keys()), FLAGS.ckpt_path, data_path)
